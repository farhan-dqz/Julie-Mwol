...
const LOAD_ING = "```බාගනිමින් පවතියි😻...```"
...
Julie.addCommand({ pattern: 'insta ?(.*)', fromMe: false, desc: "ඉන්ස්ටග්‍රෑම් වලින් බාගැනීම", dontAddCommandList: true }, async (message, match) => { 
    ...
    if (type === 'image') return await message.sendMessage(data, MessageType.image, { caption: "💫Made By 👨‍💻x-ᴅᴇᴠɪʟ🇱🇰", quoted: message.data })
    if (type === 'video') return await message.sendMessage(data, MessageType.video, { caption: "💫Made By 👨‍💻x-ᴅᴇᴠɪʟ🇱🇰", quoted: message.data })
});
